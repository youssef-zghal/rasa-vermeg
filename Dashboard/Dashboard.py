...
def update_dropdown_options(fournisseur):
    if fournisseur:
        options = [{'label': fournisseur_, 'value': fournisseur_} for fournisseur_ in fournisseurs_df['Fournisseur']]
    else:
        options = []  # Si aucune donnée n'est reçue, les options sont vides
    print("Options du dropdown mises à jour:", options)  # Ajouter cette ligne pour imprimer les options
    return options
